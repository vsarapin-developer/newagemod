    ...
    internal static class Changelog
    {
        ...
        private static readonly Entry[] Entries =
        {
            new Entry
            {
                ...
                Lines = new[]
                {
                    "СЫРОЕ, проверяй осторожно. Запасной набор: в настройках мода появился манекен, в нём мышкой собирается комплект из сумки, надетого и хранилища. После сдачи вещей в хранилище мод сам надевает этот набор, при возврате снимает и возвращает прежние вещи по своим слотам. Вещи ложатся в те слоты, которые реально займёт сервер, поэтому манекен показывает то, что окажется на персонаже. Пользуйся на спокойном бою и проверяй, что всё вернулось: механика новая и обкатана мало.",
                    "СЫРОЕ, проверяй осторожно. Идущие хаотические бои в списке заявок: карточка перечёркнута крестом, нажатие уводит смотреть бой. Мод убирает карточку, когда бой заканчивается при тебе, сверяет список при входе в комнату, снимает протухшую карточку, если по ней нажали и бой не открылся, и возвращает тебя из просмотра через пару секунд после конца боя. Показываются они только в своей комнате: хаос в хаосе, турниры в турнирах. Тоже свежая часть, возможны шероховатости.",
                    "Умения списком в бою, как в старом клиенте: полоса значков справа вверху, между полосой фазы и панелью бойца. Нажатие выбирает умение, дальше клик по бойцу применяет его на него; умения на себя срабатывают сразу. Курсор меняется на прицел, клетка цели подсвечивается зелёным, если цель годится, и красным, если нет; сама модель подсвечивается. Отмена — правой кнопкой, Escape или кнопкой с крестом под полосой. Кнопка «Умения» в нижней панели при этом прячется, а ряд смыкается. Включается галкой в настройках, по умолчанию выключено.",
                    "Значки умений показывают состояние: перезарядка идёт круговой заливкой с числом оставшихся раундов, недоступное умение затемнено. Проверка цели — игровая: мод спрашивает у игры, годится ли выбранный боец, и не даёт применить туда, куда игра не даёт.",
                    "Подсказка по бойцу стала вдвое компактнее: колонки по содержимому, длинные имена обрезаются, вместо слов «жизнь», «мана», «энергия» — значки. Энергию видно у себя и у союзников. Наведение переделано: подсказка ловится по силуэту модели, не показывается на мобах и петах и не мешает, пока выбрано умение. Имя источника эффекта, которого нет в бою, мод спрашивает у сервера, вместо номера.",
                    "Окно «Эффекты» уменьшено примерно вдвое, а пока оно включено, игровая колонка состояний выбранного бойца справа скрыта.",
                    "«Кто в игре»: группировка по кланам с заголовками и числом игроков, растягивание окна по ширине, починен поиск — раньше после смены локации фильтр и поле расходились, и список мог остаться пустым. Если в настройках указан тот же персонаж, которым играешь, окно не открывается и мод предупреждает плашкой.",
                    "Всплывающие сообщения командного чата показывают адресата: «отправитель → кому», а обращение к тебе выделяется зелёным.",
                    "У звуков мода появился свой ползунок громкости, не связанный с громкостью игры. Заменённые игровые звуки по-прежнему молчат.",
                    "Камера в бою: ползунок «Отдаление» позволяет отвести её дальше игрового предела, приближение остаётся игровым. Галкой бой можно начинать сразу отдалённой камерой.",
                    "Остаток банок на кнопке обновляется сразу после покупки, а не в следующий пересчёт.",
                    "Поход к точке карты объясняет отказ: если по дороге есть неоткрытые точки, мод пишет их номера, а ожидание ответа сервера срезано с пяти минут до тридцати секунд.",
                    "Кнопки смены оружия и реликвий больше не блокируют друг друга: второе нажатие встаёт в очередь и срабатывает само.",
                    "Контрприём на себя теперь включён по умолчанию, обновление выключено, интервал два раунда.",
                    "Escape закрывает окна мода по одному: выбор банки, выбор цели умения, окна набора — и только потом игровое меню.",
                },
            },
            new Entry
            {
