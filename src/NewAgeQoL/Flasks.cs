...
namespace NewAgeQoL
{
    internal static class Flasks
    {
        ...
        private static IEnumerator ScanRoutine()
        {
            ...
            // При открытой сумке запрос списка вкладок (339) игра принимает и на СВОЮ панель, а её
            // TabPanel старые кнопки не убирает — получается второй ряд вкладок со стрелками. Поэтому
            // спрашиваем только вкладку «Все»: не тот таб — игра ответ просто игнорирует.
            List<int> tabs;
            if (BagOpen())
            {
        ...
        // Соединение пересоздаётся при перезаходе, и подписки на старом объекте умирают вместе с ним:
        // помним, к какому именно мы подписались, и перевешиваемся на новый.
        ...
        // Ответ пришёл не на наш запрос, а на игровой (игрок листает сумку): бесплатно освежаем
        // остаток тех предметов, которые в нём упомянуты. Под фильтром поиска содержимое урезано —
        // такие ответы не считаем, иначе счётчик показал бы ноль.
